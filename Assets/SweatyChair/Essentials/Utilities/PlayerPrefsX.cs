...
namespace SweatyChair
{
	public static class PlayerPrefsX
	{
		...
		/// Stores a Int Array or Multiple Parameters into a Key
		/// </summary>
		public static bool Set2DIntArray(string key, int[,] intArray)
		{
			if (intArray == null || intArray.Length == 0) {
				PlayerPrefs.DeleteKey(key);
				return false;
			...
			System.Text.StringBuilder sb = new System.Text.StringBuilder();
			for (int i = 0, imax = intArray.GetLength(0); i < imax; i++) {
				for (int j = 0, jmax = intArray.GetLength(1); j < imax; j++)
					sb.Append(intArray[i,j]).Append(",");
				sb.Append("|");
			}
			// Notes there's a extra comma at end of each row, extra separator at end
			// 1,2,3,|4,5,6,|7,8,9,|
		...
		/// Returns a Int Array from a Key
		/// </summary>
		public static int[,] Get2DIntArray(string key)
		{
			if (PlayerPrefs.HasKey(key)) {
				string[] rowArray = PlayerPrefs.GetString(key).Split('|');
				string[] firstRowColumnArray = rowArray[0].Split(',');
				int[,] intArray = new int[rowArray.Length - 1, firstRowColumnArray.Length - 1]; // One extra comma each row and extra separator at end
				for (int i = 0, imax = rowArray.Length - 1; i < imax; i++) {
					string[] columnArray = rowArray[i].Split(',');
					for (int j = 0, jmax = firstRowColumnArray.Length - 1; j < jmax; j++)
						intArray[i, j] = Convert.ToInt32(columnArray[j]);
				}
				return intArray;
			}
			return new int[0,0];
		...
		/// <summary>
		/// Returns a 2D Int Array from a Key
		/// Note: Uses default values to initialize if no key was found
		/// </summary>
		public static int[,] Get2DIntArray(string key, int defaultValue, int defaultRows, int defaultColumns)
		{
			if (PlayerPrefs.HasKey(key))
				return Get2DIntArray(key);
			int[,] intArray = new int[defaultRows, defaultColumns];
			for (int i = 0; i < defaultRows; i++) {
				for (int j = 0; j < defaultColumns; j++)
					intArray[i,j] = defaultValue;
			}
			return intArray;
		}
