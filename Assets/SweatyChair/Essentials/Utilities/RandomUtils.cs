...
namespace SweatyChair
{
	...
	public static class RandomUtils
	{
		...
		{
			return Range((SCRandom)null, min, max);
		}
		...
		public static float Value(this System.Random rng)
		{
		...
		{
			if (FlipACoin(headsFavour))
				OnHeadsAction();
			else
				OnTailsAction();
		}
		public static bool FlipACoin(float headsFavour = 0.5f)
		{
			if (headsFavour > 1 || headsFavour < 0) {
				headsFavour = Mathf.Clamp01(headsFavour);
				Debug.LogError("[RandomUtils] Heads favour should be a number between 0 and 1.");
			}

			return (headsFavour != 0) && (Random.Range(0f, 1f) <= headsFavour);
		}
		public static bool FlipACoin(System.Random rng, float headsFavour = 0.5f)
		{
			if (headsFavour > 1 || headsFavour < 0) {
				headsFavour = Mathf.Clamp01(headsFavour);
			...
			return (headsFavour != 0) && (rng.Range(0f, 1f) <= headsFavour);
		}
		...
		#region Random Arrays / Lists / Enumerables
		...
		{
			return data.ElementAt(Random.Range(0, data.Count()));
		}


		public static T GetRandom<T>(this T[] data, System.Random rng)
		{
			return data[rng.Range(0, data.Length)];
		}

		public static T GetRandom<T>(this List<T> data, System.Random rng)
		{
			return data[rng.Range(0, data.Count)];
		}

		public static T GetRandom<T>(this IEnumerable<T> data, System.Random rng)
		{
			return data.ElementAt(rng.Range(0, data.Count()));
		}

		#endregion

		#region Random Arrays / Lists / Enumerables Index

		public static int GetRandomIndex<T>(this T[] data)
		{
			return Random.Range(0, data.Length);
		}

		public static int GetRandomIndex<T>(this List<T> data)
		{
			return Random.Range(0, data.Count);
		}

		public static int GetRandomIndex<T>(this IEnumerable<T> data)
		{
			return Random.Range(0, data.Count());
		}


		public static int GetRandomIndex<T>(this T[] data, System.Random rng)
		{
			return rng.Range(0, data.Length);
		}

		public static int GetRandomIndex<T>(this List<T> data, System.Random rng)
		{
			return rng.Range(0, data.Count);
		}

		public static int GetRandomIndex<T>(this IEnumerable<T> data, System.Random rng)
		{
			return rng.Range(0, data.Count());
		}
