...
namespace SweatyChair.TreeView
{
	...
	public static class TableViewUtility
	{

		#region Tree <--> List

		public static void TreeToList<T>(T root, IList<T> result) where T : TableViewElement
		{
			if (result == null)
		...
		public static T ListToTree<T>(IList<T> list) where T : TableViewElement
		{
			// Validate input
			...
			for (int parentIndex = 0; parentIndex < list.Count; parentIndex++) {
				...
				// Fill child array
				List<TableViewElement> childList = null;
				if (childCount != 0) {
					childList = new List<TableViewElement>(childCount); // Allocate once
					childCount = 0;
					for (int i = parentIndex + 1; i < list.Count; i++) {
						if (list[i].depth == parentDepth + 1) {
		...
		public static void ValidateDepthValues<T>(IList<T> list) where T : TableViewElement
		{
			if (list.Count == 0)
		...
		public static void UpdateDepthValues<T>(T root) where T : TableViewElement
		{
			if (root == null)
		...
		static bool IsChildOf<T>(T child, IList<T> elements) where T : TableViewElement
		{
			while (child != null) {
		...
		public static IList<T> FindCommonAncestorsWithinList<T>(IList<T> elements) where T : TableViewElement
		{
			if (elements.Count == 1)
